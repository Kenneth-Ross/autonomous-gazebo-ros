...
class CombinedStreamer(Node):
    def __init__(self):
        ...
        self.rgb_sub = message_filters.Subscriber(
            self, Image, '/sim/camera/rgb/image_raw', qos_profile=sub_qos)
        self.depth_sub = message_filters.Subscriber(
            self, Image, '/sim/camera/depth/image_raw', qos_profile=sub_qos)
        
        # Standard Subscribers JUST for diagnostics
        self.rgb_diag_sub = self.create_subscription(
            Image, '/sim/camera/rgb/image_raw', self.rgb_diag_cb, sub_qos)
        self.depth_diag_sub = self.create_subscription(
            Image, '/sim/camera/depth/image_raw', self.depth_diag_cb, sub_qos)
