...
Superseded by [`AGENTS.md`](AGENTS.md) and [`docs/CODEX_WORKFLOWS.md`](docs/CODEX_WORKFLOWS.md). The remainder is historical context.
...
> Codex operates in the **server environment**, not the Orange Pi (edge device). Edge-side verification must be run by the user unless the device is explicitly available, and results must be based on returned output.
>
> **Documentation Strategy Memory:**
> Codex must update relevant project docs and plans alongside implementation; no unavailable `documenter` subagent is required.
>
> **Remote Repository Sync Memory:**
> Edge changes need an explicit deployment handoff. Commit and push only when the user asks.
