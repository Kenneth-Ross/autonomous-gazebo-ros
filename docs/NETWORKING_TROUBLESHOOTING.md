...
> **Current troubleshooting guide.** Incident results are labeled historical; validate deployed values against [STREAMING_CONTRACT.md](STREAMING_CONTRACT.md).
...
- A single 3840x800 packed super-frame is approximately **9 MB** raw.
...
## 3. Historical Fixes and Current Checks

### Historical Fix 1: Dedicated Ethernet Subnet
...
### Historical Fix 2: Physical Link & Auto-Negotiation Diagnostics (NO-CARRIER)
...
### Historical Fix 3: Kernel Parameter Tuning
...
### Historical Fix 4: Static Peer Discovery & Network Interface Binding
...
### Historical Fix 5: Strict Parameter Typing
...
## 5. Historical NPU Inference & ROS 2 Integration Issues
...
**Root Cause:**
- A legacy unpacker published `/edge/camera/depth/camera_info` as Best Effort while the processor requested Reliable, preventing delivery. The current C++ decoder path must be inspected with `ros2 topic info -v` rather than assumed to share that profile.
**Fix:**
- Aligned the subscription QoS profile to `pipeline_qos` (Best Effort) inside `cone_landmark_processor.py`.

---

## 6. Historical Network Benchtest (iperf3)
...
3. `ros2 topic hz /edge/camera/rgb/image_raw` (Check high-bandwidth flow)
