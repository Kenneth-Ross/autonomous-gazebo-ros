...
- Bounded approximate-sync restored mapping and moving `map -> base_link`, but conversion still ranged from 0.2-2.8 seconds. Dynamically setting `wait_for_transform=0.0` did not change the distribution and produced no TF/data failures, rejecting a single CoreWrapper odometry-wait cause. Landmark batches were still camera-framed, requiring CoreWrapper to transform every persistent detection. The processor now performs one camera-to-`base_link` lookup per frame and publishes RTAB-Map landmarks in `base_link`; map-frame persistence remains unchanged. Orange Pi performance rerun pending.
