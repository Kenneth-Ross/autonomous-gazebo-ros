...
Landmark poses remain relative to `camera_link_optical`; converting them to `base_link` changed their observed height and is not part of the interface. CoreWrapper uses `wait_for_transform=0.0`, so a transform unavailable at a landmark timestamp is rejected immediately instead of blocking the RGB-D conversion callback.
