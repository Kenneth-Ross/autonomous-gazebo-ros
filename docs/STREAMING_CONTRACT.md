...
Wire publishers use reliable KeepLast(2). Edge transport subscriptions request best-effort KeepLast(2) from those compatible publishers, preventing stale reliable backlog when codec rates differ. ROS 2 Jazzy `zstd_image_transport` 4.0.7 decompresses image metadata and pixels but omits `CompressedImage.header` when constructing the decoded `Image`. The edge decoder therefore reads the standard Zstd wire payload directly with libzstd, validates metadata and decoded size, and copies the wire header exactly; malformed payloads increment the malformed counter and are dropped. Receiver callbacks only enqueue shared messages. A worker matches exact timestamps in a configurable bounded window (`pairing_queue_depth`, default 8), evicts only on capacity, drops old incomplete data, and publishes the newest complete pair. Edge raw
