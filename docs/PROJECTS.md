...
| `[VERIFICATION]` | RTAB-Map SLAM map, odometry, and `map -> base_link` | Moving transform and `/rtabmap/mapData` publication observed; odometry ownership and namespace corrected; landmark-enabled Orange Pi acceptance captured 0.727 Hz, 1.732 ms mean/2.600 ms p95 conversion, 237.382 ms mean RTAB-Map processing, and 310.632 ms mean delay while camera pairing held 30 FPS; promoted cone heights remained 0.085-0.257 m | Add deterministic and adversarial SLAM acceptance coverage before `[COMPLETE]` |
