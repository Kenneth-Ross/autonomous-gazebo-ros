...
| `[W.I.P]` | RTAB-Map SLAM map, odometry, and `map -> base_link` | Moving transform and `/rtabmap/mapData` publication observed; odometry ownership and namespace corrected; repeatable performance probe measured a 0.471 Hz post-C++ baseline with 1515 ms mean/2640 ms p95 conversion; direct filtered-odometry input implemented to remove redundant timestamped TF lookups | Verify conversion-rate improvement on Orange Pi and retain final full-stack SLAM acceptance evidence |
