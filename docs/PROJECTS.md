...
| `[W.I.P]` | RTAB-Map SLAM map, odometry, and `map -> base_link` | Moving transform and `/rtabmap/mapData` publication observed; odometry ownership and namespace corrected; landmark-enabled baseline measured 0.471 Hz and 1515 ms mean/2640 ms p95 conversion; disabling landmark processing restored 0.908 Hz and 1.076 ms mean/1.700 ms p95 conversion, isolating landmark transform ingestion; direct filtered odometry and non-blocking CoreWrapper transform lookup are implemented | Verify landmark-enabled conversion improvement, correct cone heights, and final full-stack SLAM acceptance on Orange Pi |
