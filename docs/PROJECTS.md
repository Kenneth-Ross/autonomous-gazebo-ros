...
| `[VERIFICATION]` | Edge RTAB-Map | [Acceptance specification](../plans/RTABMAP_SLAM_ACCEPTANCE.md) | Decoder, odometry, EKF, SLAM composition, deterministic evaluator, and nominal/adversarial hardware runners exist | Retain two nominal and one adversarial Orange Pi pass |
...
| `[VERIFICATION]` | RTAB-Map SLAM map, odometry, and `map -> base_link` | Moving transform and `/rtabmap/mapData` publication observed; landmark-enabled Orange Pi acceptance captured 0.727 Hz and 2.600 ms p95 conversion while camera pairing held 30 FPS; automated nominal/adversarial threshold coverage and hardware runner added | Retain two fixed-route nominal passes and one pause/recovery adversarial pass before `[COMPLETE]` |
